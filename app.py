...
with st.sidebar:
        """
         За допомогою цього веб-застосунку Ви зможете отримати прогноз для бізнесу роздрібної торгівлі. Програма розрахована на малий та середній бізнес, який уже має певний набір даних про свої продажі, але не використовує його для аналізу та прогнозування. 
         Для цього зробіть наступні кроки: 
         1. Імпортуйте набір даних 
         2. Виберіть горизонт прогнозу 
         3. Ознайомтеся із візуалізацією даних 
         4. Завантажуйте прогноз
...
if df is not None:
    ...
    """
    Насупний графік відорбражає залежність дати (ds) від цільової характеристики (y) + прогнозований інтервал.
    ...
    """
    Наступні графіки відображають тренд для цільової характеристики відносно дати і періодичну кореляцію.
    """
    fig2 = m.plot_components(forecast)
    st.write(fig2)
    
    st.info('👇 Інтерактивні графіки')

    """    
    За допомогою інтерактивних графіків Ви можете:
    * приблизити необхідний період для детальнішого перегляду
